...
function validateImportedStatePayload(importedState) {
  ...
  Object.entries(IMPORT_STATUS_ENUMS).forEach(([field, allowed]) => {
    if (
      ...
      errors.push(
        `${fieldLabel(field)} has unsupported status "${importedState[field]}".`,
      );
    }
  });
  ...
  if (errors.length) {
    throw new Error(
      `Import validation failed: ${errors.slice(0, 5).join(" ")}`,
    );
  }
  return true;
}
...
function validateMigratedImportState(candidate) {
  ...
  numericFields.forEach((field) => {
    if (!Number.isFinite(Number(candidate[field])))
      errors.push(
        `${fieldLabel(field)} is not a finite number after migration.`,
      );
  });
  if (
    ...
    errors.push(
      `Crew size must be between ${MIN_CREW_SIZE} and ${MAX_CREW_SIZE}.`,
    );
  }
  const actionIds = new Set(actions.map((action) => action.id));
  Object.values(candidate.plannedActions || {}).forEach((actionId) => {
    if (!actionIds.has(actionId))
      errors.push(`Unknown planned action "${actionId}".`);
  });
  Object.values(candidate.confirmedActions || {}).forEach((actionId) => {
    if (!actionIds.has(actionId))
      errors.push(`Unknown confirmed action "${actionId}".`);
  });
  [
    ...(candidate.ongoing || []),
    ...(candidate.deferredCompletions || []),
  ].forEach((item) => {
...
function validateImportedNumberRange(object, field, min, max, errors) {
  if (
    !hasImportField(object, field) ||
    !isFiniteNumericImportValue(object[field])
  )
    return;
...
function validateImportedStringArray(
  value,
  label,
  errors,
  { required = false } = {},
...
function validateImportedCrewPayload(crew, errors) {
  ...
  crew.forEach((character, index) => {
    ...
    validateImportedText(
      character.name,
      `Crew entry ${index + 1} name`,
      errors,
    );
    validateImportedText(
      character.lastAction,
      `Crew entry ${index + 1} last action`,
      errors,
    );
    ["labor", "exhaustion"].forEach((field) => {
      if (
        ...
        errors.push(
          `Crew entry ${index + 1} ${fieldLabel(field)} must be a finite number.`,
        );
      if (
        character[field] !== undefined &&
        isFiniteNumericImportValue(character[field]) &&
        ...
        errors.push(
          `Crew entry ${index + 1} ${fieldLabel(field)} must be 0 or greater.`,
        );
    });
    [
...
function validateImportedKnowledgePayloads(importedState, errors) {
  if (isPlainImportObject(importedState.playerKnowledge)) {
    Object.entries(importedState.playerKnowledge).forEach(([key, value]) => {
      ...
      if (
        ...
        errors.push(
          `Player knowledge value for ${key} must be text, numeric, or null.`,
        );
      }
    });
  }
  if (isPlainImportObject(importedState.waterKnowledge)) {
    validateImportedText(
      importedState.waterKnowledge.turnKey,
      "Water knowledge turn key",
      errors,
    );
    validateImportedText(
...
function validateImportedConditionsPayload(conditions, errors) {
  ...
  conditions.forEach((condition, index) => {
    ...
    if (
      ...
      !isFiniteNumericImportValue(condition.turns)
    ) {
...
function validateImportedPendingChecksPayload(pendingChecks, errors) {
  ...
  pendingChecks.forEach((prompt, index) => {
    ...
    ["reveals", "successReveals", "failureReveals"].forEach((field) =>
      validateImportedStringArray(
        prompt[field],
        `${promptLabel} ${fieldLabel(field)}`,
        errors,
      ),
    );
      ...
      if (!Array.isArray(prompt.outcomes)) {
        ...
        prompt.outcomes.forEach((outcome, outcomeIndex) => {
          if (!isPlainImportObject(outcome)) {
            errors.push(
              `${promptLabel} outcome ${outcomeIndex + 1} must be an object.`,
            );
            return;
          }
          validateImportedText(
...
function validateImportedRestMealStatus(restMealStatus, errors) {
  ...
  validateImportedBoolean(
    restMealStatus.pending,
    "Rest meal status pending flag",
    errors,
  );
  ...
  ["dinnerKey", "breakfastKey"].forEach((field) =>
    validateImportedText(
      restMealStatus[field],
      `Rest meal status ${field}`,
      errors,
    ),
  );
}
...
function loadState() {
  const raw =
    localStorage.getItem("openSeaTracker") ||
    localStorage.getItem("openSeaTrackerDraft");
  if (raw) {
...
function devValidationChecks() {
  ...
  withState(
    { crew: structuredClone(defaultState.crew).slice(0, MIN_CREW_SIZE) },
    () => {
      check(
  ...
  withState(
    {
      crew: [
        ...structuredClone(defaultState.crew),
        defaultCrewMember(6, "Player 7"),
      ],
    },
    () => {
      check(
  ...
  withState({ courseMeter: 6 }, () => {
    ...
    check(
      "Navigate failure adds Course +2",
      state.courseMeter === 8,
  ...
  withState(
    ...
    () => {
      ...
      check(
        ...
        state.travelTicks === 44 &&
          !state.pendingChecks.some(
            (prompt) => prompt.title === "Helm Check",
          ) &&
          (state.log || "").includes("Mast broken:"),
  ...
  withState({ day: 1, turn: 8 }, () => {
    ...
    check(
      "Day 1 Turn 8 skips Open Sea Event",
      openSeaEventRequiredForTurn() === false,
    );
  });
  withState({ day: 2, turn: 7 }, () => {
    ...
    check(
      ...
      scripted?.title === "Nereids Encounter" &&
        scripted.blocksOpenSea === true,
      scripted?.title || "missing",
    );
  });
  withState({ day: 2, turn: 8 }, () => {
    ...
    check(
      ...
      scripted?.title === "Bev Nightmare Encounter" &&
        scripted.blocksOpenSea === true,
      scripted?.title || "missing",
    );
  });
  withState({ day: 1, turn: 2 }, () => {
    check(
      "Day 1 Turn 2 rolls Open Sea Event",
      openSeaEventRequiredForTurn() === true,
    );
  });
  withState({ day: 1, turn: 3 }, () => {
    check(
      "Day 1 Turn 3 skips Open Sea Event",
      openSeaEventRequiredForTurn() === false,
    );
  });
  ...
  withState({ waterLevel: 3, minIngress: 2, activeLeaks: 1 }, () => {
    ...
    const prompt = state.pendingChecks.find(
      (item) => item.effect === "bilgeRod",
    );
    check(
  ...
  withState(
    { day: 1, turn: 1, waterLevel: 3, minIngress: 2, activeLeaks: 1 },
    () => {
  ...
  withState({}, () => {
    ...
    const prompt = state.pendingChecks.find(
      (item) => item.effect === "navigateCourse",
    );
    check(
      "Navigate prompt carries a visible DC",
      prompt?.dc === 15,
      `got ${prompt?.dc}`,
    );
  });
  withState({}, () => {
    ...
    const prompt = state.pendingChecks.find(
      (item) => item.effect === "harpoon",
    );
    check(
      "Harpoon prompt carries a visible DC",
      prompt?.dc === 18,
      `got ${prompt?.dc}`,
    );
  });
  ...
  withState({ conditions: [{ name: "Floating Wreckage", turns: 1 }] }, () => {
    check(
      ...
      isActionDropdownAvailable(
        defaultState.crew[0].name,
        actionById("recoverWreckage"),
      ),
    );
  });
  ...
  try {
    ...
    check(
      "Import rejects unsafe object keys",
      false,
      "unsafe key was accepted",
    );
  } catch {
  ...
  try {
    ...
    check(
      "Import rejects unknown planned action ids",
      false,
...
function migrateState() {
  ...
  const hadTravelTicks = Object.prototype.hasOwnProperty.call(
    incomingState,
    "travelTicks",
  );
  ...
  if (typeof state.mast === "boolean")
    state.mastStatus = state.mast ? "Repaired" : "Broken";
  if (typeof state.rudder === "boolean")
    state.rudderStatus = state.rudder ? "Repaired" : "Broken";
  if (typeof state.pump === "boolean")
    state.pumpStatus = state.pump ? "Working" : "Jammed";
  if (typeof state.net === "boolean")
    state.netStatus = state.net ? "Ready" : "Tangled";
  if (typeof state.rigging === "boolean")
    state.riggingStatus = state.rigging ? "Intact" : "Broken";
  ...
  state.crew = Array.from({ length: targetCrewSize }, (_, index) => {
    ...
    const existing =
      incomingCrew[index] ||
      incomingCrew.find((c) => c.name === defaultName) ||
      {};
    ...
    return {
      ...
      cartographerToolsProficiency: Boolean(
        existing.cartographerToolsProficiency,
      ),
    };
  });
  ...
  state.waterKnowledge.streak = Math.max(
    0,
    Number(state.waterKnowledge.streak || 0),
  );
  state.waterKnowledge.knownThisTurn = Boolean(
    state.waterKnowledge.knownThisTurn,
  );
  state.pendingChecks = Array.isArray(state.pendingChecks)
    ? state.pendingChecks
    : [];
  state.deferredCompletions = Array.isArray(state.deferredCompletions)
    ...
    : [];
  state.consumedMeals =
    state.consumedMeals && typeof state.consumedMeals === "object"
      ? state.consumedMeals
      : {};
  state.appliedScriptedEvents =
    state.appliedScriptedEvents &&
    typeof state.appliedScriptedEvents === "object"
      ? state.appliedScriptedEvents
      : {};
  state.longRestLaborRecoveryPending = Boolean(
    state.longRestLaborRecoveryPending,
  );
  ...
  state.turnStep = Math.max(
    1,
    Math.min(turnSteps.length, Number(state.turnStep || 1)),
  );
...
function log(message) {
  state.log =
    `Day ${state.day}, Turn ${state.turn}: ${message}\n` + (state.log || "");
}
