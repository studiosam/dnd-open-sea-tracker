...
const IMPORT_ARRAY_FIELDS = ['crew', 'conditions', 'ongoing', 'pendingChecks', 'deferredCompletions'];
...
function importStateFile(file){
  ...
  if (Number(file.size || 0) > IMPORT_MAX_BYTES) {
    alert(`Import failed: ${file.name || 'Selected file'} is too large. Maximum supported size is ${formatNumber(IMPORT_MAX_BYTES / 1024)} KB.`);
    return;
  }
...
function validateImportedStatePayload(importedState){
  ...
  IMPORT_NUMERIC_FIELDS.forEach(field => {
    if (Object.prototype.hasOwnProperty.call(importedState, field) && !isFiniteNumericImportValue(importedState[field])) {
      errors.push(`${fieldLabel(field)} must be a finite number.`);
    }
  });
  IMPORT_OBJECT_FIELDS.forEach(field => {
    if (Object.prototype.hasOwnProperty.call(importedState, field) && !isPlainImportObject(importedState[field])) {
      errors.push(`${fieldLabel(field)} must be an object.`);
    }
  });
  IMPORT_ARRAY_FIELDS.forEach(field => {
    if (Object.prototype.hasOwnProperty.call(importedState, field) && !Array.isArray(importedState[field])) {
      errors.push(`${fieldLabel(field)} must be an array.`);
    }
  });
  Object.entries(IMPORT_STATUS_ENUMS).forEach(([field, allowed]) => {
    if (Object.prototype.hasOwnProperty.call(importedState, field) && !allowed.includes(importedState[field])) {
  ...
  validateImportedWorkItemsPayload(importedState.deferredCompletions, 'Deferred completion', errors);
...
function validateMigratedImportState(candidate){
  const errors = [];
  const numericFields = ['day','turn','travel','travelTicks','courseMeter','waterLevel','minIngress','activeLeaks','food','freshWater','repairMaterials','salvagedTimber','turnStep','waterTravelPenalty'];
  numericFields.forEach(field => {
    if (!Number.isFinite(Number(candidate[field]))) errors.push(`${fieldLabel(field)} is not a finite number after migration.`);
  });
  if (!Array.isArray(candidate.crew) || candidate.crew.length < MIN_CREW_SIZE || candidate.crew.length > MAX_CREW_SIZE) {
  ...
  [...(candidate.ongoing || []), ...(candidate.deferredCompletions || [])].forEach(item => {
    if (item?.actionId && !actionIds.has(item.actionId)) errors.push(`Unknown ongoing action "${item.actionId}".`);
  });
  if (errors.length) {
...
function validateImportedRanges(importedState, errors){
  [
    ...
    ['overtimeTurnCount', 0]
  ].forEach(([field, min]) => validateImportedNumberRange(importedState, field, min, Infinity, errors));
  validateImportedNumberRange(importedState, 'courseMeter', COURSE_METER_MIN, COURSE_METER_MAX, errors);
...
function validateImportedNumberRange(object, field, min, max, errors){
  ...
  if (value < min || value > max) {
    errors.push(Number.isFinite(max)
...
function validateImportedText(value, label, errors, {required = false, maxLength = IMPORT_TEXT_MAX_LENGTH} = {}){
...
function validateImportedStringArray(value, label, errors, {required = false} = {}){
  ...
  value.forEach((entry, index) => validateImportedText(entry, `${label} entry ${index + 1}`, errors));
}

function validateImportedEnum(value, label, allowed, errors){
  if (value !== undefined && !allowed.includes(value)) errors.push(`${label} has unsupported value "${value}".`);
}
...
function validateImportedCrewPayload(crew, errors){
  ...
  crew.forEach((character, index) => {
    ...
    ['labor', 'exhaustion'].forEach(field => {
      if (character[field] !== undefined && !isFiniteNumericImportValue(character[field])) errors.push(`Crew entry ${index + 1} ${fieldLabel(field)} must be a finite number.`);
      if (character[field] !== undefined && isFiniteNumericImportValue(character[field]) && Number(character[field]) < 0) errors.push(`Crew entry ${index + 1} ${fieldLabel(field)} must be 0 or greater.`);
    });
    [
    ...
    ].forEach(field => validateImportedBoolean(character[field], `Crew entry ${index + 1} ${fieldLabel(field)}`, errors));
  });
}
...
function validateImportedObjectMaps(importedState, errors){
  ['plannedActions', 'confirmedActions', 'overtimeExhaustion'].forEach(field => {
    ...
    Object.entries(map).forEach(([key, value]) => {
      ...
      if (field === 'overtimeExhaustion' && !isFiniteNumericImportValue(value)) errors.push(`${fieldLabel(field)} value for ${key} must be numeric.`);
      if (field !== 'overtimeExhaustion' && typeof value !== 'string') errors.push(`${fieldLabel(field)} value for ${key} must be an action id.`);
    });
  });
}
...
function validateImportedKnowledgePayloads(importedState, errors){
  ...
  if (isPlainImportObject(importedState.waterKnowledge)) {
    ...
    validateImportedText(importedState.waterKnowledge.lastKnownTurnKey, 'Water knowledge last known turn key', errors);
    validateImportedBoolean(importedState.waterKnowledge.knownThisTurn, 'Water knowledge known-this-turn flag', errors);
    if (importedState.waterKnowledge.streak !== undefined) {
      if (!isFiniteNumericImportValue(importedState.waterKnowledge.streak)) errors.push('Water knowledge streak must be numeric.');
      else if (Number(importedState.waterKnowledge.streak) < 0) errors.push('Water knowledge streak must be 0 or greater.');
    }
  }
}
...
function validateImportedWorkItemsPayload(items, label, errors){
  ...
  items.forEach((item, index) => {
    ...
    validateImportedText(item.actionId, `${itemLabel} action id`, errors, {required:true});
    validateImportedStringArray(item.actors, `${itemLabel} actors`, errors, {required:true});
    ...
    ['remaining', 'createdDay', 'createdTurn'].forEach(field => {
      if (item[field] !== undefined && !isFiniteNumericImportValue(item[field])) errors.push(`${itemLabel} ${fieldLabel(field)} must be a finite number.`);
      if (item[field] !== undefined && isFiniteNumericImportValue(item[field]) && Number(item[field]) < 0) errors.push(`${itemLabel} ${fieldLabel(field)} must be 0 or greater.`);
    });
  });
}
...
function validateImportedPendingChecksPayload(pendingChecks, errors){
  ...
  pendingChecks.forEach((prompt, index) => {
    ...
    validateImportedText(prompt.id, `${promptLabel} id`, errors, {required:true});
    validateImportedText(prompt.character, `${promptLabel} character`, errors);
    validateImportedText(prompt.title, `${promptLabel} title`, errors, {required:true});
    validateImportedText(prompt.detail, `${promptLabel} detail`, errors, {required:true});
    ...
    if (prompt.dc !== undefined && (!isFiniteNumericImportValue(prompt.dc) || Number(prompt.dc) < 0)) errors.push(`${promptLabel} DC must be a finite number 0 or greater.`);
    ['reveals', 'successReveals', 'failureReveals'].forEach(field => validateImportedStringArray(prompt[field], `${promptLabel} ${fieldLabel(field)}`, errors));
    if (prompt.outcomes !== undefined && prompt.outcomes !== null) {
      if (!Array.isArray(prompt.outcomes)) {
        ...
        prompt.outcomes.forEach((outcome, outcomeIndex) => {
          ...
          validateImportedText(outcome.id, `${promptLabel} outcome ${outcomeIndex + 1} id`, errors, {required:true});
          validateImportedText(outcome.label, `${promptLabel} outcome ${outcomeIndex + 1} label`, errors, {required:true});
          validateImportedText(outcome.className, `${promptLabel} outcome ${outcomeIndex + 1} class`, errors);
        });
      }
    }
  });
...
function validateImportedTurnLedger(turnLedger, errors){
  ...
  ['pumping', 'buckets'].forEach(field => {
    if (turnLedger[field] !== undefined && !isFiniteNumericImportValue(turnLedger[field])) errors.push(`Turn ledger ${fieldLabel(field)} must be numeric.`);
    if (turnLedger[field] !== undefined && isFiniteNumericImportValue(turnLedger[field]) && Number(turnLedger[field]) < 0) errors.push(`Turn ledger ${fieldLabel(field)} must be 0 or greater.`);
  });
}
...
function validateImportedRestMealStatus(restMealStatus, errors){
  ...
  ['dinnerAvailable', 'breakfastAvailable'].forEach(field => {
    if (restMealStatus[field] !== undefined && restMealStatus[field] !== null && typeof restMealStatus[field] !== 'boolean') {
      errors.push(`Rest meal status ${field} must be true, false, or null.`);
    }
  });
  ['dinnerKey', 'breakfastKey'].forEach(field => validateImportedText(restMealStatus[field], `Rest meal status ${field}`, errors));
}
...
function devValidationChecks(){
  ...
  check('Initial defaults', Number(defaultState.travelTicks) === 44 && Number(defaultState.travel) === 5.5 && Number(defaultState.courseMeter) === 12 && Number(defaultState.minIngress) === 1 && Number(defaultState.activeLeaks) === 0 && Number(defaultState.repairMaterials) === 4, 'Travel 44 ticks/5.5 days, Course Meter 12, min ingress 1, leaks 0, repairs 4.');
  ...
  withState({crew:structuredClone(defaultState.crew).slice(0, MIN_CREW_SIZE)}, () => {
    check('Crew migration allows minimum size', state.crew.length === MIN_CREW_SIZE, `got ${state.crew.length}`);
  });
  withState({crew:[...structuredClone(defaultState.crew), defaultCrewMember(6, 'Player 7')]}, () => {
    check('Crew migration allows maximum size', state.crew.length === MAX_CREW_SIZE, `got ${state.crew.length}`);
  });
  withState({crew:Array.from({length:MAX_CREW_SIZE + 1}, (_, index) => defaultCrewMember(index, `Crew ${index + 1}`))}, () => {
    check('Crew migration clamps above maximum', state.crew.length === MAX_CREW_SIZE, `got ${state.crew.length}`);
  });
  check('Course Meter ranges', courseStateForMeter(12).name === 'True Course' && courseStateForMeter(9).name === 'Drifting' && courseStateForMeter(6).name === 'Off Course' && courseStateForMeter(3).name === 'Lost');
  check('Travel tick conversion', daysToTravelTicks(5.5) === 44 && travelDaysFromTicks(1) === 0.125, `${daysToTravelTicks(5.5)} ticks for 5.5 days.`);

  state = {travel:4.75};
  migrateState();
  check('Old decimal travel migrates to ticks', state.travelTicks === 38 && state.travel === 4.75, `got ${state.travelTicks} ticks / ${state.travel} days`);
  state = originalState;
  ...
  withState({courseMeter:6}, () => {
    ...
    check('Navigate reveals resulting Course State', state.playerKnowledge.courseState === 'Drifting', `got ${state.playerKnowledge.courseState}`);
  });
  withState({courseMeter:10}, () => {
    applyNavigateResult('navigateCriticalSuccess');
    check('Navigate critical success caps Course Meter', state.courseMeter === 12, `got ${state.courseMeter}`);
  });
  withState({courseMeter:12}, () => {
    decayCourseMeter();
    check('Course Meter decays without navigation', state.courseMeter === 11, `got ${state.courseMeter}`);
  });
  withState({courseMeter:12}, () => {
    applyNavigateResult('navigateCriticalFailure');
    decayCourseMeter();
    check('Navigate skips end-of-turn Course decay', state.courseMeter === 12 && state.navigateResolvedThisTurn === true && (state.log || '').includes('decay was skipped'), `course ${state.courseMeter}`);
  });
  withState({courseMeter:12, travelTicks:44}, () => {
    applyHelmTravelOutcome('helmSuccess');
    check('Helm success applies current Course State', state.travelTicks === 42, `got ${state.travelTicks}`);
  });
  withState({courseMeter:6, travelTicks:44}, () => {
    applyHelmTravelOutcome('helmCriticalSuccess');
    check('Helm critical success improves Course State', state.travelTicks === 43 && state.courseMeter === 7, `ticks ${state.travelTicks}, course ${state.courseMeter}`);
  });
  withState({courseMeter:12, travelTicks:44}, () => {
    applyHelmTravelOutcome('helmFailure');
    check('Helm failure applies one worse Course State', state.travelTicks === 43 && state.courseMeter === 12, `ticks ${state.travelTicks}, course ${state.courseMeter}`);
  });
  withState({courseMeter:5, travelTicks:44}, () => {
    applyHelmTravelOutcome('helmCriticalFailure');
    check('Helm critical failure worsens travel and reduces Course Meter', state.travelTicks === 45 && state.courseMeter === 4, `ticks ${state.travelTicks}, course ${state.courseMeter}`);
  });
  withState({mastStatus:'Working', rudderStatus:'Working', courseMeter:12, travelTicks:44}, () => {
    applyActionStart(state.crew[0], actionById('helm'));
    const helmPrompt = state.pendingChecks.find(prompt => prompt.title === 'Helm Check' && prompt.type === 'check');
    check('Functional mast and rudder create normal Helm Check', Boolean(helmPrompt), `prompts ${state.pendingChecks.map(prompt => prompt.title).join(', ')}`);
  });
  withState({mastStatus:'Broken', rudderStatus:'Working', courseMeter:12, travelTicks:44}, () => {
    applyActionStart(state.crew[0], actionById('helm'));
    check('Broken mast prevents Helm Check prompt', !state.pendingChecks.some(prompt => prompt.title === 'Helm Check'), `prompts ${state.pendingChecks.map(prompt => prompt.title).join(', ')}`);
    check('Broken mast produces no Helm travel progress', state.travelTicks === 44, `got ${state.travelTicks}`);
    check('Broken mast logs automatic Helm result', (state.log || '').includes('Mast broken:'), 'missing mast broken log');
  });
  withState({mastStatus:'Broken', rudderStatus:'Broken', courseMeter:12, travelTicks:44}, () => {
    applyActionStart(state.crew[0], actionById('helm'));
    check('Both broken uses broken mast behavior', state.travelTicks === 44 && !state.pendingChecks.some(prompt => prompt.title === 'Helm Check') && (state.log || '').includes('Mast broken:'), `ticks ${state.travelTicks}`);
  });
  withState({mastStatus:'Working', rudderStatus:'Broken', courseMeter:12, travelTicks:44}, () => {
    ...
    check('Broken rudder prevents Helm Check prompt', !state.pendingChecks.some(prompt => prompt.title === 'Helm Check'), `prompts ${state.pendingChecks.map(prompt => prompt.title).join(', ')}`);
    check('Broken rudder automatically applies random Course State travel', [-2, -1, 0, 1].includes(delta), `delta ${delta}`);
    check('Broken rudder logs random applied state', (state.log || '').includes('Random applied state:'), 'missing rudder broken log');
  });
  withState({mastStatus:'Working', rudderStatus:'Broken', courseMeter:0, travelTicks:44}, () => {
    const originalRandom = Math.random;
    Math.random = () => 0;
    try {
      applyActionStart(state.crew[0], actionById('helm'));
    ...
    check('Broken rudder random movement ignores Course Meter', state.travelTicks === 42, `got ${state.travelTicks} ticks with Lost course meter`);
  });
  ...
  withState({waterLevel:3, minIngress:2, activeLeaks:1}, () => {
    ...
    check('Bilge Rod success reveals Water Level and Total Ingress', Boolean(prompt?.successReveals?.includes('waterLevel') && prompt?.successReveals?.includes('totalIngress')));
    check('Bilge Rod reveals Water Level on failure', Boolean(prompt?.failureReveals?.includes('waterLevel') && !prompt?.failureReveals?.includes('totalIngress')));
  });
  withState({day:1, turn:1, waterLevel:3, minIngress:2, activeLeaks:1}, () => {
    ...
    state.turn = 2;
    rememberPlayerKnowledge('waterLevel');
    check('Total Ingress reveals after two known water turns', firstTurnIngress === null && state.playerKnowledge.totalIngress === 3, `first ${firstTurnIngress}, second ${state.playerKnowledge.totalIngress}`);
  });
  withState({}, () => {
  ...
  check('Ingress severity scales by active leaks', activeLeaksSeverityClass(0) === 'good' && activeLeaksSeverityClass(1) === 'warn' && activeLeaksSeverityClass(3) === 'danger');
  check('Total ingress severity compares against minimum', totalIngressSeverityClass(2, 2) === 'good' && totalIngressSeverityClass(3, 2) === 'warn' && totalIngressSeverityClass(4, 2) === 'danger');
  ...
  try {
    const normalized = normalizeImportedState({travel:4.75, crew:structuredClone(defaultState.crew)});
    check('Import normalizes old decimal travel', normalized.travelTicks === 38 && normalized.travel === 4.75, `ticks ${normalized.travelTicks}, days ${normalized.travel}`);
  } catch (error) {
    check('Import normalizes old decimal travel', false, error.message);
  }
  try {
    normalizeImportedState({plannedActions:{Leopold:'notAnAction'}, crew:structuredClone(defaultState.crew)});
...
function migrateState(){
  ...
  state.travelTicks = hadTravelTicks && Number.isFinite(parsedTravelTicks)
