...
const events = {
  ...
  19: {title:'Favorable Winds', text:'Travel Time -0.25 days (-2 ticks) if mast and rudder are functional.', apply:s => {
...
function defaultCrewName(index){ return crewNames[index] || `Player ${index + 1}`; }
...
function mastFunctional(){ return ['Working','Repaired'].includes(state.mastStatus); }
function rudderFunctional(){ return ['Working','Repaired'].includes(state.rudderStatus); }
function canUseNormalHelm(){ return mastFunctional() && rudderFunctional(); }
function canTravel(){ return canUseNormalHelm(); }
...
function daysToTravelTicks(days){ return Math.max(0, Math.round(Number(days || 0) * TRAVEL_TICKS_PER_DAY)); }
function travelDaysFromTicks(ticks = state.travelTicks){ return Number(ticks || 0) / TRAVEL_TICKS_PER_DAY; }
function syncTravelDaysFromTicks(){ state.travel = travelDaysFromTicks(); }
...
function formatTravelDays(ticks = state.travelTicks){ return `${formatNumber(travelDaysFromTicks(ticks))}d`; }
function signedTicks(value){ return `${Number(value) > 0 ? '+' : ''}${Number(value || 0)}`; }
function applyCourseMeterChange(amount, reason){
  ...
  if (state.courseMeter !== before || amount) log(`${reason}: Course Meter changed from ${before} to ${state.courseMeter}.`);
}
function raiseCourseMeterOneState(reason){
  ...
  log(`${reason}: Course improved from ${currentState.name} to ${betterState}. Course Meter changed from ${before} to ${state.courseMeter}.`);
}
function decayCourseMeter(){
  if (state.navigateResolvedThisTurn) {
    log('End-of-turn Course Meter decay was skipped because Navigate / Study Map was resolved this turn.');
    return;
  }
  const before = clampCourseMeter(state.courseMeter);
  state.courseMeter = Math.max(COURSE_METER_MIN, before - 1);
  if (state.courseMeter !== before) log(`End-of-turn Course Meter decay changed Course Meter from ${before} to ${state.courseMeter}.`);
}
function applyHelmTravelOutcome(outcome){
  const currentState = courseStateForMeter();
  const appliedStateName = outcome === 'helmCriticalSuccess'
    ? betterCourseStateName(currentState.name)
    : outcome === 'helmSuccess' ? currentState.name : worseCourseStateName(currentState.name);
  ...
  log(`Helm used ${appliedStateName}; Travel Remaining changed from ${formatTravelDays(beforeTicks)} (${beforeTicks} ticks) to ${formatTravelDays()} (${state.travelTicks} ticks).`);
}
function applyAutomaticHelmSystemOutcome(name){
  ...
  if (!rudderFunctional()) {
    ...
    const beforeTicks = Number(state.travelTicks || 0);
    applyTravelTicks(deltaTicks);
    log(`Rudder broken: ${name} wrestled the helm, but the Marrowwind answered unpredictably. Random applied state: ${appliedStateName}. Travel changed by ${signedTicks(deltaTicks)} tick${Math.abs(deltaTicks) === 1 ? '' : 's'} from ${formatTravelDays(beforeTicks)} (${beforeTicks} ticks) to ${formatTravelDays()} (${state.travelTicks} ticks).`);
    return true;
  }
  return false;
}
...
function advantageSourceText(name, type){
  ...
  return sources.length ? ` This character rolls with advantage from ${sources.join(' and ')}.` : '';
}
function fogHelmText(name){ return denseFogActive() ? (hasHelmAdvantage(name) ? ' Dense Fog disadvantage is canceled by this character\'s helm advantage, so roll normally.' : ' Dense Fog: roll this Survival check at disadvantage.') : ''; }
function hasFishermanBackground(name){ return Boolean(crewByName(name)?.fishermanBackground); }
function fishingAdvantageText(names, eventAdvantageText = ''){
  ...
  if (fisherfolk.length) {
    parts.push(`${fisherfolk.join(', ')} ${fisherfolk.length === 1 ? 'rolls' : 'roll'} with advantage from Fisherman background.`);
  }
