...
function syncFromInputs(){
  ['day','turn','waterLevel','minIngress','activeLeaks','food','freshWater','repairMaterials','salvagedTimber','courseMeter'].forEach(id => {
    const element = q(id);
...
function voyageScoreCard(){
  ...
  return {
    ...
    metrics: [
      ...
      {label:'Travel Ticks', value:state.travelTicks, controls:scoreMiniControls('travelTicks', 1) + metricKnowledgeControls('travel', 'Reveal Travel Remaining'), detail:formatTravelDays(), featured:true},
      {label:'Course', value:state.courseMeter, controls:scoreMiniControls('courseMeter', 1) + metricKnowledgeControls('courseState', 'Reveal Course'), detail:courseState.name}
    ],
    ...
    sections: [
      {
        ...
        metrics: [
          {label: 'Current Water Level', value: state.waterLevel, controls: scoreMiniControls('waterLevel', 1), detail: knowledgeStatus('waterLevel'), featured: true}
        ],
      ...
      },
      {
        ...
        metrics: [
          {label: 'Base Ingress', value: state.minIngress, controls: scoreMiniControls('minIngress', 1), className: 'good'},
          {label: 'Active Leaks', value: state.activeLeaks, controls: scoreMiniControls('activeLeaks', 1), className: activeLeaksSeverityClass()},
          {label: 'Total Ingress', value: totalIngress(), detail: knowledgeStatus('totalIngress'), className: totalIngressSeverityClass()}
        ],
...
function turnStatusScoreCard(context){
  ...
  return {
    ...
    metrics: [
      {label: 'Checks', value: context.unresolvedChecks, detail: scoreCheckDetail(context.pendingPreAction, context.pendingAction, context.pendingWater)},
...
function suppliesScoreCard(){
  const className = Number(state.food) <= 1 || Number(state.freshWater) <= 1
    ? 'danger'
    : Number(state.repairMaterials) <= 1 ? 'warn' : '';
  return {
    label: 'Supplies',
    className: `supplies-score-item ${className}`,
    metricClassName: 'score-metric-grid supplies-metric-grid',
    metrics: [
      {label: 'Food', value: formatNumber(state.food), controls: scoreMiniControls('food', 0.25) + metricKnowledgeControls('food', 'Reveal Food'), detail: knowledgeStatus('food')},
      {label: 'Water', value: formatNumber(state.freshWater), controls: scoreMiniControls('freshWater', 0.25) + metricKnowledgeControls('freshWater', 'Reveal Fresh Water'), detail: knowledgeStatus('freshWater')},
      {label: 'Repairs', value: state.repairMaterials, controls: scoreMiniControls('repairMaterials', 1) + metricKnowledgeControls('repairMaterials', 'Reveal Repair Supplies'), detail: knowledgeStatus('repairMaterials')}
    ]
  };
}
...
function systemsScoreCard(){
  ...
  return {
    ...
    footer: {
      ...
      summary: failed ? systemEffects.join(' · ') : 'Normal helm, pumping, net fishing, and deck movement.'
    }
  };
}
...
function failedSystemEffects(){
  return [
    ...
    state.rudderStatus === 'Broken' && state.mastStatus !== 'Broken' ? 'Rudder: helm applies random course drift' : '',
    state.rudderStatus === 'Broken' && state.mastStatus === 'Broken' ? 'Rudder: random drift suppressed while mast is broken' : '',
...
function activePlayEffects(){
  ...
  if (!mastFunctional()) {
    effects.push({title:'Mast Broken', detail:'Man Helm is automatic: no Helm check is made and the ship cannot make sail progress.', className:'danger'});
  } else if (!rudderFunctional()) {
    effects.push({title:'Rudder Broken', detail:'Man Helm is automatic: no Helm check is made and the ship randomly applies True Course, Drifting, Off Course, or Lost.', className:'danger'});
  }
  return effects;
}
