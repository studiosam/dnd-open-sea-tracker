...
function forceScriptedSceneTurn(){
  ...
  state.pendingChecks = state.pendingChecks.filter(prompt => prompt.phase !== 'action' || prompt.status === 'resolved');
  state.startedGroups = {};
  state.actionsCommittedThisTurn = true;
  state.scriptedSceneTurn = true;
  state.turnStep = 5;
  log('This turn was handled as a scripted scene. Normal crew actions were forced to Idle and ongoing work will pause until the next normal turn.');
  saveStateSnapshot();
  render();
}
...
function setCrewProficiency(index, field, hasProficiency){
  const character = state.crew[index];
  if (!character || !['fishermanBackground','waterVehiclesProficiency','navigatorToolsProficiency','cartographerToolsProficiency'].includes(field)) return;
  pushUndo(`Changed ${character.name}'s ${fieldLabel(field)}`);
  character[field] = Boolean(hasProficiency);
  log(`Crew setup: ${character.name} ${character[field] ? 'has' : 'does not have'} ${fieldLabel(field)}.`);
  saveStateSnapshot();
  render();
}
...
function removeCrewReferences(name){
  ...
  Object.keys(state.playerKnowledge || {}).forEach(key => {
    if (state.playerKnowledge[key] && typeof state.playerKnowledge[key] === 'object' && state.playerKnowledge[key].character === name) {
      delete state.playerKnowledge[key];
    }
  });
  state.ongoing.forEach(item => { item.actors = (item.actors || []).filter(actor => actor !== name); });
  state.deferredCompletions.forEach(item => { item.actors = (item.actors || []).filter(actor => actor !== name); });
  Object.keys(state.startedGroups || {}).forEach(actionId => {
    state.startedGroups[actionId] = (state.startedGroups[actionId] || []).filter(actor => actor !== name);
  ...
  state.deferredCompletions = state.deferredCompletions.filter(item => (item.actors || []).length);
}
...
function pruneCrewScopedState(){
  ...
  state.ongoing = state.ongoing
    .map(item => ({...item, actors:(item.actors || []).filter(name => activeCrewNames.has(name))}))
    .filter(item => item.actors.length);
  state.deferredCompletions = state.deferredCompletions
    .map(item => ({...item, actors:(item.actors || []).filter(name => activeCrewNames.has(name))}))
    .filter(item => item.actors.length);
  state.pendingChecks = state.pendingChecks.filter(prompt => !prompt.character || activeCrewNames.has(prompt.character));
  Object.keys(state.startedGroups || {}).forEach(actionId => {
    state.startedGroups[actionId] = (state.startedGroups[actionId] || []).filter(name => activeCrewNames.has(name));
    if (!state.startedGroups[actionId].length) delete state.startedGroups[actionId];
  });
}
...
function maybeAddActionPrompt(character, action){
  ...
  if (action.check === 'helm') {
    ...
    const criticalSuccessText = improvedState === courseState.name
    ...
    const criticalSuccessLabel = improvedState === courseState.name
      ? `Critical Success: ${improvedState}`
      : `Critical Success: ${improvedState} + Course State`;
    addPrompt({type:'check', character:name, title:'Helm Check', detail:`Survival DC 12. ${criticalSuccessText} Success applies ${courseState.name} (${signedTicks(courseState.deltaTicks)} travel ticks). Failure applies ${worsenedState} (${signedTicks(courseStateTravelDeltaTicks(worsenedState))} travel ticks) for this turn only. Critical failure also reduces Course Meter by 1.${advantageSourceText(name, 'helm')}${fogHelmText(name)}`, dc:12, effect:'helmCourse', outcomes:[
      ...
      {id:'helmCriticalFailure', label:`Critical Fail: ${worsenedState} + Course -1`, className:'danger'}
    ]});
  }
  if (action.check === 'navigate') {
    addPrompt({type:'check', character:name, title:'Navigate / Study Map', detail:`Resolve the navigation check. Critical failure: Course Meter +0. Failure: +2. Success: +4. Critical success: +5. Course Meter caps at 12. Players learn the resulting Course State whether the check succeeds or fails.${advantageSourceText(name, 'navigate')}`, dc:15, effect:'navigateCourse', reveals:['courseState'], outcomes:[
  ...
  if (action.check === 'bilgeRod') {
    addPrompt({
      ...
      detail:'Investigation DC 15. Any reading gives the current Water Level. Success also reveals Total Water Ingress. Total Water Ingress can also become known if players know Water Level two turns in a row.',
      dc:15,
  ...
  if (action.check === 'castNet' && firstConfirmedForGroup(name, action.id)) {
    ...
    const advantage = fishingAdvantageText(actors, hasFishAdvantage ? 'School of Fish grants advantage to this Cast Net action.' : '');
    addPrompt({type:'check', title:'Cast Fishing Net', detail:`Both players roll Survival DC 15. Each success grants +0.25 days rations.${advantage}${fishingFogText(actors, hasFishAdvantage)}`, dc:15, effect:'castNetSuccess', outcomes:[
  ...
  if (action.check === 'harpoon') {
    ...
    const actors = [name];
    const advantage = fishingAdvantageText(actors, hasShadowAdvantage ? 'Large Shadow grants advantage to this Harpoon Fishing check.' : '');
    addPrompt({type:'check', character:name, title:'Harpoon Fishing', detail:`Survival check. DC 18: +0.5 Food. DC 20: +1 Food.${advantage}${fishingFogText(actors, hasShadowAdvantage)}`, dc:18, effect:'harpoon', outcomes:[
  ...
  if (action.check === 'assistHarpoon' && firstConfirmedForGroup(name, action.id)) {
    ...
    const actors = selectedForAction(action.id);
    const advantage = fishingAdvantageText(actors, hasShadowAdvantage ? 'Large Shadow grants advantage to this Harpoon Fishing check.' : '');
    addPrompt({type:'check', title:'Harpoon Fishing (Cooperative)', detail:`Both players roll Survival; only the higher roll counts. DC 15: +0.5 Food. DC 18: +1 Food. DC 20: +2 Food.${advantage}${fishingFogText(actors, hasShadowAdvantage)}`, dc:15, effect:'assistHarpoon', outcomes:[
      {id:'fail', label:'Fail', className:'danger'},
      {id:'foodHalf', label:'+0.5 Food', className:'good'},
      {id:'foodOne', label:'+1 Food', className:'good'},
...
function rememberPlayerKnowledge(key){
  ...
  const value = key === 'totalIngress' ? totalIngress()
    : key === 'travel' ? travelDaysFromTicks()
    : key === 'courseState' ? courseStateForMeter().name
    : Number(state[key]);
...
function applyPromptEffect(prompt, result){
  ...
  if (prompt.effect === 'helmSuccess' && result === 'success') applyHelmTravelOutcome('helmSuccess');
...
function advanceTurn(doRender = true, shouldSync = true, shouldTickOngoing = true){
  ...
  else if (shouldTickOngoing && state.scriptedSceneTurn) log('Ongoing work did not tick down because this was a scripted scene turn.');
...
function handleDelegatedClick(event){
  ...
  switch (action) {
    case 'undo-last-change': return undoLastChange();
    case 'save-state': return saveState();
    case 'load-state': return loadState();
    case 'export-state': return exportState();
    case 'choose-import-file': return chooseImportFile();
    case 'reset-state': return resetState();
    case 'change-field': return change(control.dataset.field, amount());
    case 'interrupt-selected-work': return interruptSelectedWork();
    case 'end-turn': return endTurn();
    case 'advance-turn': return advanceTurn();
    case 'advance-turn-no-ongoing-tick': return advanceTurn(true, true, false);
    case 'meal': return meal(control.dataset.meal);
    case 'long-rest': return longRest();
    case 'start-turn-prompts': return startTurnPrompts();
    case 'add-manual-prompt': return addManualPrompt();
    case 'clear-resolved-prompts': return clearResolvedPrompts();
    case 'roll-open-sea-event': return rollOpenSeaEvent();
    case 'resolve-event': return resolveEvent();
    case 'add-fog': return addFog();
    case 'clear-conditions': return clearConditions();
    case 'change-crew-size': return changeCrewSize(amount());
    case 'run-dev-validator': return runDevValidator();
    case 'scoreboard-change': return scoreboardChange(control.dataset.field, amount());
    case 'scoreboard-set': return scoreboardSet(control.dataset.field, control.dataset.value);
    case 'reveal-player-knowledge': return revealPlayerKnowledge(control.dataset.field);
    case 'forget-player-knowledge': return forgetPlayerKnowledge(control.dataset.field);
    case 'run-scripted-step': return runScriptedStep();
    case 'force-scripted-scene-turn': return forceScriptedSceneTurn();
    case 'go-to-turn-step': return goToTurnStep(Number(control.dataset.step));
    case 'set-unset-actions-to-idle': return setUnsetActionsToIdle();
    case 'confirm-all-actions': return confirmAllActions();
    case 'continue-night-overtime': return continueNightOvertime();
    case 'start-night-overtime': return startNightOvertime();
    case 'end-night-overtime-and-rest': return endNightOvertimeAndRest();
    case 'change-overtime-turn-count': return changeOvertimeTurnCount(amount());
    case 'clear-overtime-exhaustion': return clearOvertimeExhaustion();
    case 'crew-change': return crewChange(index(), control.dataset.field, amount());
    case 'confirm-character-action': return confirmCharacterActionByIndex(index());
    case 'clear-character-action': return clearCharacterActionByIndex(index());
    case 'resolve-prompt-outcome': return resolvePromptOutcome(control.dataset.promptId, control.dataset.outcomeId);
    case 'resolve-prompt': return resolvePrompt(control.dataset.promptId, control.dataset.result);
    default:
...
function handleDelegatedChange(event){
  ...
  const index = Number(control.dataset.index || 0);
  switch (action) {
    case 'rename-crew-member': return renameCrewMember(index, control.value);
    case 'set-crew-background': return setCrewBackground(index, control.checked);
    case 'set-crew-proficiency': return setCrewProficiency(index, control.dataset.field, control.checked);
    case 'set-planned-action': return setPlannedAction(index, control.value);
    default:
